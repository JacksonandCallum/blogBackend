...
public class FileController {
    ...
    private static final String emotionPath = System.getProperty("user.dir") + "/emotions/";
    ...
    public Dict emotionUpload(MultipartFile file) {
        String flag;
        synchronized (FileController.class) {
            flag = System.currentTimeMillis() + "";
            ThreadUtil.sleep(1L);
        ...
        String fileName = file.getOriginalFilename();
        try {
            if (!FileUtil.isDirectory(emotionPath)) {
                FileUtil.mkdir(emotionPath);
            }
            // 文件存储形式：时间戳-文件名
            FileUtil.writeBytes(file.getBytes(), emotionPath + flag + "-" + fileName);  // ***/manager/files/1697438073596-avatar.png
            System.out.println(fileName + "--上传成功");
